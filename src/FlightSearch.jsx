    ...
    fetch(`https://my.api.mockaroo.com/flights.json?key=0848b890`)
      ...
      .then(data => {
        const filteredFlights = data.filter(flight => 
          ...
          (!departureTime || new Date(flight.itineraries[0].departureAt).getTime() >= new Date(departureTime).getTime()) &&
          (!arrivalTime || new Date(flight.itineraries[0].arrivalAt).getTime() <= new Date(arrivalTime).getTime())
        );
